...
$this->title = Yii::t('main', 'Update Product Char');
$this->params['breadcrumbs'][] = ['label' => Yii::t('main', 'Product Chars'), 'url' => ['index']];
...
{
    $lang_model = \common\models\Product CharsLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\Product CharsLang();
        $action = "/services-lang/create";

    } else {

        $lang_model = \common\models\Product CharsLang::findOne($lang_model->id);
        $action = "/Product Chars-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/Product Chars-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
